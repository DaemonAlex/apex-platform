...
function addContact() { form.value.contacts.push({ name: '', role: '', email: '', phone: '' }); }
function removeContact(i: number) { form.value.contacts.splice(i, 1); }
...
const columns: DataTableColumns<any> = [
  ...
  { title: 'Type', key: 'type', width: 100, render: (row) => h(NTag, { type: row.type === 'internal' ? 'info' : 'default', size: 'small', bordered: true }, () => row.type) },
  { title: 'Contacts', key: 'contacts', width: 100, align: 'center', render: (row) => h('span', {}, (row.contacts || []).length) },
  { title: 'Projects', key: 'projectCount', width: 80, align: 'center', render: (row) => row.projectCount || 0 },
  {
    title: 'Actions', key: 'actions', width: 200,
    render: (row) => h(NSpace, { size: 'small' }, () => [
      h(NButton, { size: 'small', secondary: true, onClick: () => openDetail(row) }, () => 'View'),
...
<div>
  <NSpace justify="space-between" align="center" style="margin-bottom:16px;">
    <NGrid :x-gap="12" :y-gap="12" :cols="3" style="min-width:360px;">
  ...
  <NSpace style="margin-bottom:12px;" align="center">
    <NInput v-model:value="search" placeholder="Search vendors..." clearable style="width:240px;" size="small">
      <template #prefix><i class="ph ph-magnifying-glass" /></template>
    </NInput>
    <NSelect v-model:value="typeFilter" :options="typeOptions" placeholder="Type" clearable style="width:130px;" size="small" />
