...
# Example outputs (values elided — use the command above to generate your own):
# JWT_SECRET: <64-hex-characters>
# DB_PASSWORD: use a password manager to generate a 20+ character password
```
...
2. **Update Database:**
   ```sql
   ALTER LOGIN SA WITH PASSWORD = '<new password from password manager>';
   ```

3. **Update Environment:**
   ```bash
   # Update node/.env — values elided
   DB_PASSWORD=<new password from password manager>
   JWT_SECRET=<generated by the command above>
   ```
